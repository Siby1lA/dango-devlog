...
import { getKoreaTodayDateInfo } from 'utils/survey'
...
export default function Ask() {
  ...
  const submit = async () => {
    ...
    const { fireStoreDB } = await import('../components/firebase')
    ...
    const userRef = doc(fireStoreDB, `survey/ask`)
    try {
      await setDoc(userRef, { 문의하기: { [formState]: arrayUnion(newBlog) } }, { merge: true })
      setToastView(true)
      setTimeout(() => {
        setToastView(false)
      }, 3000)
  ...
  return (
    <div>
      ...
      <div
        id="survey"
        className={`relative flex h-full w-full flex-col items-center justify-center rounded-[12px] bg-[#F9FAFB] px-6 py-12 dark:bg-[#262626]`}
      >
        {!formState ? (
          <>
          ...
          <>
            <div className="flex w-full flex-col items-center">
              ...
              <textarea
                placeholder="어떤 점이 좋았거나 아쉬웠는지 알려주세요!"
